...
class taxologyWay:
    ...
    def gen_connection_method_method(self,methodlist,pmethod=None):
        ...
        for methodline in methodlist:
            ...
            if pmethod is not None:
                ...
                self.edgelist.append(",".join([name,pmethod]))
    ...
    def gen_dot(self,basename="taxo",dot=None):
        if dot is None:
            dot = Digraph(basename)
        dot.graph_attr["rankdir"] = "TB" #"LR"
        for edge in self.edgelist:
            x = edge.split(",")
            dot.edge(x[1],x[0])
            dot.node(x[0],shape="box")
            dot.node(x[1],shape="box")
        return dot
...
def read_ymlfile(filename,showpng=True):
    wf = taxologyWay() 
