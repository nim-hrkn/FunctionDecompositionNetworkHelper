...
class DecompositionTree(object):
    ...
    def __init__(self,basename = "caus"):
        ...
        self.methodcandidates =  ["method","parts","function"] 
    ...
    def method_prefix(self,methodtype=None):
        ...
        if methodtype == "parts":
            ...
            return ""
        elif methodtype == "function":
            return "method_to_"
        else:
            print("method_prefix: unsupported",methodtype)
            raise

    def func_prefix(self,methodtype=None):
        if methodtype is None:
            methodtype ="parts"
        if methodtype=="parts":
            return "get_"
            ...
            print("func_prefix: unsupported",methodtype)
            raise 
            
    def applymethod_prefix(self,methodtype=None):
        if methodtype is None:
            methodtype = "method"
        if methodtype=="parts":
            ...
            print("applymethod_prefix: unsupported",methodtype)
            raise
...
class taxologyWay(DecompositionTree):
    ...
    def gen_connection_method_method(self,methodlist,pmethodtype=None,pmethodname=None,pnodetype=None):
        ...
        if pmethodname is None:
            for methodline in methodlist:
                ...
                methodtype = None
                method = None
                for meth in self.methodcandidates:
                    if meth in methodline.keys():
                        methodtype = meth
                        method = methodline[methodtype]
                        break
                if "nodetype" in methodline.keys():
                    nodetype = methodline["nodetype"]
                else:
                    nodetype = None
                self.gen_connection_method_method(method,methodtype,name,nodetype)
        else:
            if pnodetype == "PartOf" or pnodetype is None:
                ...
                for methodline in methodlist:
                    ...
                    for meth in self.methodcandidates:
                        if meth in methodline.keys():
                            methodtype = meth
                            method = methodline[meth]
                            break
                    #funcp->methodp edge
                    funcp = self.func_prefix(pmethodtype) + pmethodname
                    methodp = self.method_prefix(pmethodtype) + pmethodname 
                    ...
                    #methodp->func edge
                    func1 = self.func_prefix(methodtype)+ name
                ...
                applyp = self.applymethod_prefix(pmethodtype) + pmethodname
                ...
                for methodline in methodlist:
                    ...
                    methodp = self.method_prefix(pmethodtype) + pmethodname
                    func1 = self.func_prefix(methodtype) + name
                    method1 = self.method_prefix(methodtype) + name
    ...
    def methodtree(self):
        ...
        methodtype = None
        for meth in self.methodcandidates:
            if meth in data.keys():
                methodtype = meth
        self.gen_connection_method_method(data[methodtype],methodtype)
