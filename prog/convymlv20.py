...
class workflow_format_V22dev:
    ...
    def change_group(self,group):
        ...
        for g in group:
            ...
            if methodname is not None and methodtype is not None:
                dic1 = OrderedDict({ 'method': {'name':methodname, 'type':methodtype}  })
                dic.update(dic1)
            if methodname is not None and methodtype is None:
                dic1 = OrderedDict({ 'method': {'name':methodname}  })
                dic.update(dic1)

            if funcname is not None and functype is not None:
                dic1 = OrderedDict({ 'func': {'name':funcname, 'type':functype}  })
                dic.update(dic1)
            if funcname is not None and functype is None:
                dic1 = OrderedDict({ 'func': {'name':funcname}  })
                dic.update(dic1)
            groups.append(dic)
        return groups
    ...
    #    print("-glist",glist)
        glistnew = []
        for group in glist:
            groupnew = self.change_group(group["group"])
            glistnew.append({"group":groupnew})
    #    print("glistnew>",glistnew)
        #print(yaml.dump(glistnew))
        return glistnew
    ...
    def convert(self,data):
       ...
       for x in data:
           for y in data[x]:
               ...
               workflowpart = {"name":wfname,"order": "workflow", "list":glist }
               print(workflowpart)
               wflist.append(workflowpart)
